...
export default function BodyFatPage() {
  ...
  const measureGuide = lang === "en" ? [
    { label: "Neck Circumference", hint: "Just below the Adam's apple, parallel to the ground", icon: "📐" },
    { label: "Waist Circumference", hint: "At navel level, measure while exhaling", icon: "📏" },
    { label: "Hip Circumference", hint: "(Women Only) At widest point, feet together", icon: "📊" },
  ] : [
  ...
  const getCategory = (bf: number, gender: string) => {
    if (gender === "erkek") {
      if (bf < 6) return { cat: lang === "en" ? "Essential Fat" : "Esansiyel Yağ", color: "text-blue-400", bgColor: "border-blue-500/40", ideal: false, detail: lang === "en" ? "This range is not sustainable long-term. Seen only during competition season in sports." : "Bu aralık uzun süre sürdürülemez. Spor dünyasında yalnızca yarışma dönemlerinde görülür." }
      if (bf < 14) return { cat: lang === "en" ? "Athletic — Defined" : "Atletik — Define", color: "text-primary", bgColor: "border-primary/50", ideal: true, detail: lang === "en" ? "Best possible body composition for men. Visible muscle definition, vascularity beginning." : "Erkekler için olası en iyi vücut kompozisyonu. Kaslar belirgin, damarlanma başlamış." }
      if (bf < 18) return { cat: lang === "en" ? "Fit / Healthy" : "Fit / Sağlıklı", color: "text-green-400", bgColor: "border-green-500/40", ideal: true, detail: lang === "en" ? "Muscle lines visible, abs starting to show. Target range for most men." : "Kas hatları görünür, karın kasları belirginleşmeye başlar. Çoğu erkek için hedef aralık." }
      if (bf < 25) return { cat: lang === "en" ? "Acceptable" : "Kabul Edilebilir", color: "text-yellow-400", bgColor: "border-yellow-500/40", ideal: false, detail: lang === "en" ? "Low health risk but fat is visually noticeable. Cutting work is beneficial." : "Sağlık riski düşük ama görsel olarak yağ belirgin. Definasyon çalışması fayda sağlar." }
      return { cat: lang === "en" ? "Obese" : "Obezite", color: "text-red-400", bgColor: "border-red-500/40", ideal: false, detail: lang === "en" ? "Increased health risks. A structured nutrition and exercise program should be started." : "Sağlık riskleri artmış. Yapılandırılmış beslenme ve egzersiz programı başlatılmalı." }
    } else {
      if (bf < 14) return { cat: lang === "en" ? "Essential Fat" : "Esansiyel Yağ", color: "text-blue-400", bgColor: "border-blue-500/40", ideal: false, detail: lang === "en" ? "Dangerously low range for women. Can lead to hormonal problems." : "Kadınlar için tehlikeli düşük aralık. Hormonal sorunlara yol açabilir." }
      if (bf < 21) return { cat: lang === "en" ? "Athletic — Defined" : "Atletik — Define", color: "text-primary", bgColor: "border-primary/50", ideal: true, detail: lang === "en" ? "Ideal range for female athletes. Strong look and healthy hormone balance." : "Kadın atletler için ideal aralık. Güçlü görünüm ve sağlıklı hormon dengesi." }
      if (bf < 25) return { cat: lang === "en" ? "Fit / Healthy" : "Fit / Sağlıklı", color: "text-green-400", bgColor: "border-green-500/40", ideal: true, detail: lang === "en" ? "Healthy and sustainable range. Ideal target zone for most women." : "Sağlıklı ve sürdürülebilir aralık. Çoğu kadın için ideal hedef bölge." }
      if (bf < 32) return { cat: lang === "en" ? "Acceptable" : "Kabul Edilebilir", color: "text-yellow-400", bgColor: "border-yellow-500/40", ideal: false, detail: lang === "en" ? "Overall health risks low but above the ideal range." : "Genel sağlık riskleri düşük fakat ideal aralığın üstünde." }
      return { cat: lang === "en" ? "Obese" : "Obezite", color: "text-red-400", bgColor: "border-red-500/40", ideal: false, detail: lang === "en" ? "High health risks. Starting with professional support is recommended." : "Sağlık riskleri yüksek. Profesyonel destek ile başlamak önerilir." }
    }
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col">
      <SEO
        title={lang === "en" ? "Body Fat % Calculator | Lean Mass Analysis" : "Vücut Yağ Oranı Hesaplayıcı | Yağsız Kütle Analizi"}
        description={lang === "en"
          ? "Calculate body fat percentage and lean body mass with the US Navy formula. Find your category and ideal target."
          : "ABD Donanması formülüyle vücut yağ oranını ve yağsız vücut kütleni hesapla. Hangi kategoride olduğunu ve ideal hedefini öğren."}
      ...
      <div className="relative pt-28 pb-14 overflow-hidden border-b border-white/8">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex items-center gap-2 text-white/40 font-sans text-sm mb-6">
            ...
            <span className="text-white/70">{lang === "en" ? "Body Fat %" : "Vücut Yağ Oranı"}</span>
          </div>
          <div className="flex items-start gap-5">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-display font-bold uppercase text-white leading-none mb-3">
                {lang === "en" ? "Body Fat" : "Vücut Yağ"} <span className="text-primary">{lang === "en" ? "%" : "Oranı"}</span>
              </h1>
              <p className="text-white/55 font-sans text-base max-w-xl leading-relaxed">
                {lang === "en"
                  ? "Calculate your body fat percentage with just a tape measure using the US Navy formula. A far more meaningful metric than a scale."
          ...
          <div className="flex flex-wrap gap-4 mt-7">
            {[
              { label: lang === "en" ? "Formula" : "Formül", val: "US Navy Method" },
              { label: lang === "en" ? "Tool" : "Araç", val: lang === "en" ? "Tape Measure Only" : "Sadece Mezura" },
              { label: lang === "en" ? "Accuracy" : "Doğruluk", val: "±3%" },
            ].map((pill) => (
          ...
          <div className="grid lg:grid-cols-5 gap-8 items-start">
            ...
            <div className="lg:col-span-3 space-y-5">
              <div className="bg-[#0D0D0D] border border-white/10 p-7">
                {/* Gender */}
                <div className="mb-6">
                  <label className={labelClass}>{lang === "en" ? "Gender" : "Cinsiyet"}</label>
                ...
                <form onSubmit={calculate} className="space-y-5">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <label className="font-display text-white uppercase tracking-widest text-xs">{lang === "en" ? "Height (cm)" : "Boy (cm)"}</label>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <label className="font-display text-white uppercase tracking-widest text-xs">{lang === "en" ? "Neck Circumference (cm)" : "Boyun Çevresi (cm)"}</label>
                    </div>
                    <div className="relative">
                      <input type="number" step="0.1" required value={neck} onChange={(e) => setNeck(e.target.value)} placeholder="38" className={inputClass} />
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-white/25 font-display">cm</span>
                    </div>
                    <p className="text-white/30 font-sans text-xs mt-1.5 ml-9">
                      {lang === "en" ? "Just below the Adam's apple, parallel to ground" : "Adem elmasının hemen altından, yere paralel"}
                    </p>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <label className="font-display text-white uppercase tracking-widest text-xs">{lang === "en" ? "Waist Circumference (cm)" : "Bel Çevresi (cm)"}</label>
                    </div>
                    <div className="relative">
                    ...
                    <p className="text-white/30 font-sans text-xs mt-1.5 ml-9">
                      {lang === "en" ? "At navel level, measure while exhaling" : "Göbek deliği hizasından, nefes verince ölç"}
                    </p>
                  </div>
                    ...
                    {gender === "kadin" && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <label className="font-display text-white uppercase tracking-widest text-xs">{lang === "en" ? "Hip Circumference (cm)" : "Kalça Çevresi (cm)"}</label>
                        </div>
                        <div className="relative">
                        ...
                        <p className="text-white/30 font-sans text-xs mt-1.5 ml-9">
                          {lang === "en" ? "At the widest point of the hips, feet together" : "Kalçanın en geniş yerinden, ayaklar kapalı"}
                        </p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <button type="submit" className="w-full bg-primary text-black py-4 font-display text-sm uppercase tracking-[0.2em] font-bold hover:bg-white transition-all duration-300 shadow-[0_0_20px_rgba(245,197,24,0.25)] flex items-center justify-center gap-3">
                    {lang === "en" ? "Calculate My Body Fat" : "Yağ Oranımı Hesapla"} <ArrowRight className="w-5 h-5" />
                  </button>
                </form>
              </div>
                ...
                <button onClick={() => setShowGuide(!showGuide)} className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-display text-xs uppercase tracking-widest text-white/50">
                      {lang === "en" ? "How to Measure Correctly?" : "Doğru Ölçüm Nasıl Yapılır?"}
                    </span>
                  </div>
                ...
                {result !== null && cat ? (
                  <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    ...
                    <div className={`bg-[#0D0D0D] border-2 ${cat.bgColor} p-7 text-center`}>
                      <div className="text-white/40 font-display tracking-widest uppercase text-xs mb-5">{lang === "en" ? "Your Body Fat %" : "Vücut Yağ Oranın"}</div>
                      ...
                      <div className="relative inline-flex items-center justify-center mb-5">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          ...
                          <span className="text-white/30 font-sans text-xs mt-1">{lang === "en" ? "body fat" : "yağ oranı"}</span>
                        </div>
                      </div>
                      ...
                      {cat.ideal && (
                        <div className="flex items-center justify-center gap-1.5 text-green-400/70 font-sans text-xs">
                          <CheckCircle2 className="w-3.5 h-3.5" /> {lang === "en" ? "Within target range" : "Hedef aralık içinde"}
                        </div>
                      )}
                    </div>
                    ...
                    {/* Scale bar */}
                    <div className="bg-[#0D0D0D] border border-white/8 p-5">
                      <div className="text-white/35 font-display text-xs uppercase tracking-widest mb-3">{lang === "en" ? "Scale" : "Ölçek"}</div>
                      ...
                      <div className="flex justify-between text-white/20 font-sans text-[10px]">
                        {lang === "en"
                          ? <><span>Essential</span><span>Athletic</span><span>Fit</span><span>Obese</span></>
                    ...
                    {/* Detail */}
                    <div className="bg-[#0D0D0D] border border-white/8 p-5">
                      <div className="text-white/40 font-display tracking-widest uppercase text-xs mb-3">{lang === "en" ? "Coach Commentary" : "Koç Yorumu"}</div>
                      ...
                      <div className="mt-3 pt-3 border-t border-white/8 text-white/35 font-sans text-xs">
                        {lang === "en" ? (gender === "erkek" ? "Ideal range for men:" : "Ideal range for women:") : (gender === "erkek" ? "Erkekler için" : "Kadınlar için")} {lang === "en" ? "" : "ideal aralık:"} <span className="text-white/70">%{idealMin}–{idealMax}</span>
                      </div>
                    </div>
                    ...
                    <button onClick={() => setResult(null)} className="w-full py-3 border border-white/15 text-white/50 font-display text-xs uppercase tracking-widest hover:border-white/40 transition-all">
                      {lang === "en" ? "Recalculate" : "Yeniden Hesapla"}
                    </button>
                  </motion.div>
                ) : (
                  <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#0D0D0D] border border-white/8 min-h-[380px] flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <div className="font-display text-white/25 uppercase tracking-widest text-sm mb-3">{lang === "en" ? "Measurement Result" : "Ölçüm Sonucu"}</div>
                    <p className="text-white/20 font-sans text-sm max-w-xs leading-relaxed">
                      {lang === "en"
                        ? "After entering your measurements, your body fat % will appear in the circular gauge."
                    ...
                    <div className="mt-5 flex items-center gap-2 bg-primary/8 border border-primary/20 px-4 py-2.5">
                      ...
                      <p className="text-white/40 font-sans text-xs text-left">
                        {lang === "en" ? "Far more reliable than BMI alone" : "BMI'dan çok daha güvenilir bir ölçüt"}
                      </p>
                    </div>
                  </motion.div>
                )}
              ...
              >
                <div>
                  ...
                  <h3 className="font-display text-2xl text-white uppercase mb-2">
                    {lang === "en" ? "Your Body Fat:" : "Yağ Oranın"} <span className="text-primary">%{result.toFixed(1)}</span>{cat.ideal ? (lang === "en" ? " — Maintain the Ideal" : " — İdeal Seviyeyi Koru") : (lang === "en" ? " — Get to the Target" : " — Hedef Aralığa Gel")}
                  </h3>
                  <p className="text-white/55 font-sans text-sm">
                    {cat.ideal
                      ? (lang === "en" ? "Let's create a personal plan to maintain your current form and add muscle mass." : "Mevcut formu korumak ve kas kütlesi eklemek için kişisel plan oluşturalım.")
                      : (lang === "en" ? `Target range is %${idealMin}–${idealMax}. Let's make a concrete plan to get there.` : `Hedef aralık %${idealMin}–${idealMax}. Oraya ulaşmak için somut bir plan yapalım.`)}
                  </p>
                </div>
                <Link href="/#contact">
                  <button className="flex-shrink-0 bg-primary text-black font-display uppercase tracking-widest px-7 py-3.5 hover:bg-white transition-all duration-300 text-sm flex items-center gap-2 shadow-[0_0_20px_rgba(245,197,24,0.3)]">
                    {lang === "en" ? "Free Consultation" : "Ücretsiz Danışma Al"} <ArrowRight className="w-4 h-4" />
                  </button>
                </Link>
              </motion.div>
            )}
